...
export default function TimePickerValue({ onTimeChange, onSearch }) {
    ...
    const initialStartTime = currentTime.startOf('hour').add(roundedMinutes, 'minutes');
  ...
  const [endTimeChanged, setEndTimeChanged] = useState(false); // Track if end time has been changed
   const [searchEnabled, setSearchEnabled] = useState(false);
    ...
    const handleEndTimeChange = (newEndTime) => {
        ...
        if (dayjs(newEndTime).diff(startTime, 'minutes') >= 15 && dayjs(newEndTime).isAfter(startTime)) {
        ...
        <div className="timepicker-container">
            <Paper elevation={3}>
                <div className="header" style={{ borderBottom: '1px solid #a8a8a850' }}>
                    <div className="title" style={{ fontSize: '1rem' }}>Select Your Meeting Time</div>
                </div>
                    ...
                    <div className="timepicker-wrapper">
                        <div className="timepicker-column">
                            <TimePicker
                                ...
                                inputProps={{
                                    style: { borderColor: startTimeChanged ? 'black' : 'red' } // Change border color based on start time interaction
                                }}
                            />
                        </div>
                        <div className="timepicker-column">
                            <TimePicker
                                ...
                                minutesStep={15} // Set step to 15 minutes
                                inputProps={{
                                    style: { borderColor: endTimeChanged ? 'black' : 'red' } // Change border color based on end time interaction
                                }}
                            />
                        </div>
